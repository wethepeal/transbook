...
>**npm，不是 pnpm**：本机 pnpm 的 `pnpm.exe` 被错误硬链接成了 POSIX 二进制
...
| D-014 | 2026-09-17 | **结构识别必须优先取 NAV/NCX**：样书 `<h1>`~`<h6>` 计数为 **0**（该出版社用 CSS 类名表达标题层级） | 实测发现，若依赖语义标签会导致结构全丢 | 写入计划书风险 #1（高） |
...
| D-018 | 2026-09-17 | **PDF 引擎改为 Typst**（备选 WeasyPrint 排除）：WeasyPrint 在本机 Windows 报 `libgobject-2.0-0` 缺失（需 GTK/Pango）；Typst 1.23 s 出 A5 PDF，文字抽取 5/5，渲染图人眼确认禁则/缩进/页码正确 | M0 实测 #5 | **架构影响**：内容真源由 XHTML 上提为 **IR**（EPUB→XHTML、PDF→Typst 两个适配器）；字体用本机 **Noto Serif SC / Noto Sans SC**（已装，无需下载） |
...
| D-020 | 2026-09-17 | **PDF 抽取定为 pypdfium2 单引擎**：质量与 PyMuPDF **完全等同**（156,881 字符）但**快 3 倍**、许可宽松，且**能直读书签**（12 条，与 EPUB NAV 对应）；**pdfminer.six 淘汰**（本书为 **Type3 字体**，只抽出 3%） | M0 实测 #6（373 页真实日文 PDF） | PDF 与 EPUB **共用同一「目录→章节」结构模型**。 本条原写"该样书是横排"**已勘误**（见 D-036） |
...
| D-023 | 2026-09-17 | **M1 第 5 步完成**：SQLite 段落表（稳定 `seg_id`、`text_hash` 内容寻址）+ TM（跨书复用）+ 成本/状态统计；`doc_id` 用书名短标识 | 需要断点续跑、审核回灌与成本核算的载体 | 真实样书 3401 段落入库；重复导入幂等、原文未变译文不丢、机翻不覆盖定稿；提交 `1fadf51` |
...
| D-029 | 2026-09-17 | **审核回流闭环完成**：`export-review`（TSV，只改第 4 列）→ 人工修改 → `apply-review`（按 `seg_id` 回灌）→ 重出终版；另有 `clear-review` 回滚到机翻 | 用户 Q5 要求"先双语审核、通过后出纯中文" | 安全保证：只写 `final_translation`、回灌前自动备份、留空视为不改、未知 ID 只报告。真实样书验证：3401 行 → 改 5 行 → 回灌（采纳 5/未变 3396）→ **终版 EPUB 确认含人工定稿**；57 测试全绿 |
...
| D-034 | 2026-09-20 | **DeepSeek API 接入**：密钥写入项目 `.env`（已 gitignore）；实测 6 段 → 480 入/181 出 token → **¥0.0024**；译文质量良好（`本電子書籍は縦書きでレイアウトされています。` → `本电子书采用竖排布局。`） | 用户提供密钥 | **思考模式默认开启**（effort=high），必须传 `extra_body={"thinking":{"type":"disabled"}}` 关闭，否则思维链白花 token；已固化到 `tp translate --engine deepseek` |
...
| D-044 | 2026-09-21 | **lxml 元素的 `id()` 不可作长期标识**：lxml 元素是代理对象，代理被回收后 `id()` 会被新代理复用。用 `id(el)` 做"已处理"集合会**误杀无关元素**| 实测脚注整块消失（被表格子树的回收代理 id 撞上） | 正确做法：集合里**同时持有元素强引用**，代理不被回收，`id()` 才稳定唯一 |
...
| D-064 | 2026-09-21 | **一处真实隐患：界面上「试跑（Fake 引擎）」与真翻译原本并排等权重**。Fake 引擎写入的是 `[译]原文` 这种假译文，而且**会进翻译记忆库**，之后真翻译可能复用它们 | 重构时审 UI 发现 | 已把「试跑」单独放到分隔线右侧、改 ghost 外观，并在下方说明"会进翻译记忆库、正式翻译前建议覆盖"。**根治需要引擎侧配合**（Fake 的产物不应写入 TM），已记入 §8 待办 |
...
| D-067 | 2026-09-21 | **产物文件名用「书名」而不是项目名**（`user-test.zh.epub` → `Re：ゼロから始める異世界生活 43.zh.epub`） | 用户："项目名不应该影响产物的命名，而是用作项目管理" | 项目名（`doc_id`）是**管理标识**：要能当目录名、要稳定、由用户随手取；产物是给人看的文件，发给别人得能看出是哪本书。清洗规则里有一条值得记住：非法字符**换成全角等价字符**而不是删掉——`Re:ゼロ…` 删成 `Reゼロ…` 就缺了一块，而全角冒号在 Windows 上完全合法；只有斜杠必须删（换成全角等于把路径语义引进文件名）。取不到书名时退回 `doc_id`，保证总有可用名字。实现见 `render/naming.py` |
| D-068 | 2026-09-21 | **"项目存在"的判定看的是"有没有文件"，不是"抽取跑完没有"**；**删除＝清空项目内容，保留项目行与任务日志** | 用户实测：新建项目后进详情页报「项目不存在」，像建失败了 | ① 404 是**时序**问题——上传只同步写了 `source.epub`，抽取是后台作业，而 `exists()` 当时要求已有 IR/db。这类"看起来像功能失败、实际是状态没跟上"的 bug，判据要选**用户视角的"这个东西在不在"**，而不是内部流程跑到哪一步了。② 删除后留一个 `.deleted` 标记、**不删目录**：目录一删这一行就从列表消失，用户会以为删错了东西；而任务日志在工作根的 `service.db` 里、本来就不在项目目录下，所以"保留日志"不需要额外做什么。③ 破坏性操作走**两步确认**（原地换成「确认删除/取消」，且把「取消」放在原「删除」的位置上，双击不会误触确认） |
| D-069 | 2026-09-21 | **浅色主题用 CSS 变量覆盖实现，不引入任何 per-theme 的组件分支** | 用户要求"根据现在的深色样式主题创建一个浅色主题" | 颜色本来就集中在 `styles.css` 的 `:root` 里，所以 `[data-theme='light']` 覆盖同一批变量名即可，组件一行都不用改——**两个真相来源是主题系统最常见的腐化方式**。三个具体决定：① 浅色**不是把深色取反**：深色的层级靠"更亮的表面"，浅色靠"更深的边框 + 更浅的底"，表达方式相反，必须重新定值；② 主题写在 `<html>` 上而不是某个容器上，因为 `color-scheme` 要作用于原生控件（滚动条、下拉、文件选择器），它们不属于 React 树；③ 在 `index.html` 里加一段**内联脚本**在首次绘制前定好主题——等 bundle 执行再切，选了浅色的人每次刷新都会先闪一下深色 |
| D-070 | 2026-09-21 | **发布走"推 `v*` 标签 → CI 构建并建 Release"，不需要额外权限**；**写进 README 的外链必须在同一轮里验证它真的能打开** | 用户读 README 时发现 Release 链接指向 "No releases published" | ① 机制：`release.yml` 声明 `permissions: contents: write` 并用仓库自带的 `github.token` 建 Release，所以推 tag 就够了，**不需要配 PAT 密钥**——一开始怀疑"是不是权限不够"是错的方向，真实原因是**从没推过 tag**。判断这类问题要看**触发条件**（`on: push: tags`）而不是先怀疑授权。② 更深一层的教训：我在 README 里写了 Release 下载链接，却没有验证过那个页面存在——**新加的外链要在同一轮里点开确认**，否则就是"文档承诺了不存在的东西"，用户一眼就发现。③ 首个 tag 的 `--generate-notes` 只会写一行 "Full Changelog"（前面没有可比版本），对外可见的说明要手工补 |
...
| 2026-09-21 | **新建项目误报 404 + 产物改用书名命名**（用户实测报出，D-067）：404 是**时序**问题——上传只写了 `source.epub`，而 `exists()` 当时要求已有 IR/db，抽取没跑完就 404；改名后新建项目立刻可见，列表状态分四档。产物命名抽成 `render/naming.py`，非法字符换全角（`Re:ゼロ…` → `Re：ゼロ…`）而不是删掉，实测 `ui-create-test` 项目产出 `Re：ゼロから始める異世界生活 43.zh.epub`。顺带发现产物列表混进了输入文件 `source.epub`。测试 316 → 326 | agent |
| 2026-09-21 | **列表按最近活动排序 + 删除功能 + 两处样式**（用户要求，D-068）：排序取目录里**所有文件**的最新 mtime（不是目录自身的——改 db 内容不动目录 mtime）；删除＝清空项目内容、保留项目行与**任务日志**（日志在工作根 `service.db` 里，本来就不在项目目录下），两步确认、作业在跑时 409。实测真实项目删除：26 个文件 / 17.2 MB，与磁盘完全一致。样式：项目列 234→160px（`table-layout: fixed` 才让 colgroup 生效）、状态列居中。测试 326 → 329 | agent |
| 2026-09-21 | **文档同步 + 端口说明修正 + README + 快速上手页 + 浅色主题**（用户要求）：`AGENTS.md` 整篇重写（此前还停在"项目尚未开始编码"）；`DELIVERY.md` 补 17 条 HTTP 接口清单、§4.3 删除语义、产物命名、测试数、目录树。用户指出文档把 8321 当固定地址不对——实测本机保留段含 8163-8362，8321 永远绑不上，改为"以启动时打印的地址为准"。新增 `README.md`（GitHub 门面）与网页端 `#/help` 快速上手页。新增浅色主题 + 顶栏切换（跟随系统 / localStorage 持久化 / 内联脚本防首屏闪烁），对比度实测 11 组全过；顺带修掉表单边框——两个主题下都只有 1.1~1.35:1，加 `--field-line` 到 3.09:1（WCAG 1.4.11）。清理三份文档里 138 处 emoji（`cli.py` 的 `✓`/`✗` 是排版符号，保留） | agent |
| 2026-09-21 | **发布 v0.1.0 首个 Release**（用户发现 README 里的 Release 链接指向 "No releases published"，D-070）：根因是**从来没推过 `v*` 标签**，`release.yml` 因此从未触发（本地只有一个旧的 `m0-skeleton`，不匹配 `v*`）。本地先实跑一次 `tools/build_release.py`，确认产物名与 README 写的一致（`transbook-0.1.0-win64.zip`）且脚本自校验「wheel 里真有界面」，再推 `v0.1.0` → CI run #1 **success**，两个附件都在。自动生成的说明在首个 tag 上只有一行 "Full Changelog"，另手工补成 1150 字符 | agent |
| 2026-09-21 | 用户澄清：`real-v43` 是他**测试删除功能时自己删的**，与我的操作无关（我此前如实报告了"查不出是谁删的"并给出恢复路径）。**不可恢复这一点得到了确认**，是否改成"先移进回收目录"待用户定 | agent |
