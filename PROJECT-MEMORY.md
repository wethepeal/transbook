...
| D-047 | 2026-09-21 | 🔴 **纠正 D-037 的一个错误判断：竖排 PDF 的内联振假名必须剥离**，此前注释写"对翻译无碍"是**错的** | 黄金比对（PDF vs EPUB 孪生版真值）显示：不剥注音时精确命中仅 **58.9%**，送进翻译的是 `手て強ごわい`、`効こう果か覿てき面めん` 这种被注音切碎的日文 | 用**字号**剥（正文 h≈12.2 / 注音 h≈6.1），但只靠"矮"会误删标点（`、。` h≈4.2）与促音（`っ` h≈7.4），故判据为**矮 + 是假名 + 成串**（连续≥2 个；单个则须左右都是汉字，`却って` 的 `っ` 因此保留）。实测剥离 **3277 字**后：精确命中 **58.9%→75.1%**、整书字符相似度 **0.9543→0.9955**、仅 PDF 有的段 **1240→737** |
...
| M2+ PDF 抽取保真：页眉/页脚/页码过滤 + **内联振假名剥离** | ✅ 完成（D-045/D-047）：与 EPUB 真值的整书字符相似度 **0.9955**、精确命中 **75.1%** |
...
| 2026-09-21 | **PDF 内联振假名剥离**（D-047）：纠正 D-037 的错误判断。用字号识别（正文 h≈12.2 / 注音 h≈6.1），叠加"是假名 + 成串"避开标点与促音。剥离 3277 字，**黄金比对**（vs EPUB 真值）精确命中 58.9%→**75.1%**、整书字符相似度 0.9543→**0.9955**。新增 `tools/golden_diff.py`（PDF vs EPUB 真值比对）、`tools/footnote_probe.py`、`tools/ruby_probe.py`。测试 154 → 165 全绿 | agent |
