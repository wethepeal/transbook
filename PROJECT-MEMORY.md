...
| D-065 | 2026-09-21 | 🔴 **真实 API Key 被写进测试代码并推送到远端（我的错误）**。`tests/test_config.py` 里硬编码了真实的 DeepSeek Key——来源是写"接口不能回传密钥明文"那条测试时，从一次失败的断言输出里把它复制成了测试字面量。改掉当前文件后**重写 git 历史**清除 4 个受影响提交的 blob，强推；并新增 `tools/check_secrets.py`（工作树 + `--history` 全历史）接进 CI | 用户要求"确认 GitHub 上有没有敏感信息" | 关键教训：① **只在新提交里删掉是不够的**——旧提交的 blob 里仍然留着，只有扫全历史才看得见；② 重写用了 `git filter-repo --replace-text`（`filter-branch` 的官方替代，本机 `sh` 不可用所以上次 BOM 那次才手写）；③ **验证方式是重新克隆远端再扫**，而不是只看本地——只有这样才能证明 GitHub 上真的没有了（结果：37 个提交 0 命中）；④ 密钥已推送即视为泄漏，**必须作废重发**，重写历史不能替代这一步 |
| D-066 | 2026-09-21 | 🔴 **CI 从第 5 次运行起一直失败，原因是我自己引入的**：清 ruff 时给 `pdf.py` 两处 `zip(boxes, raw)` 加了 `strict=True`，理由是"入口已保证等长"——**这个断言是错的**：`raw` 来自 `get_text_range()`、`boxes` 按 `count_chars()` 取，是两个不同 API，长度与页面断行数有关 | GitHub 告警邮件；从 API 取回真实 CI 日志确认 | 已改回 `strict=False` 并把原因写进代码。**教训：为消除 lint 告警而给代码加"更强的断言"，必须先真验证那个不变量**——我当时只是读代码推断。连带发现测试 fixture 只写了一个字体、无回退，英文 runner 上没有它→字体回退→断行变化，正是长度不等的来源；已补回退链，并在抽不出中文时 `pytest.skip`（如实跳过，而不是让 CI 长期变红）。另按各 Action 自己声明的 `runs.using` 升到首个 node24 版本，消除 Node 20 弃用告警 |
