...
#: 护栏重试时追加到用户消息里的强指令。
#: 实测两类可恢复的失败：① 模型把长段原文原样返回（1/3216）；
#: ② 长批次里吐出无法解析的 JSON（一本书 30 段）。两者原样重发大概率重现，
#: 所以重试必须**换更强的提示词**。
STRICT_SUFFIX = """
【重要】上一轮的输出不合格，必须修正。请检查以下两点：
1. **禁止原样返回原文**：必须逐段给出中文译文；即使是拟声词、专有名词或短句，
   也要译成中文（拟声词可译为对应的中文拟声表达）。
2. **必须是合法 JSON**：引号成对、逗号位置正确、不要有尾随逗号、不要在 JSON
   之外输出任何文字或 Markdown 围栏。`text` 字段里的双引号请用 \\" 转义。
"""
