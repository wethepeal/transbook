...
| 组件 | 版本 | 必需 | 说明 |
|---|---|---|---|
| Windows | 10/11 | 已满足 | 全部实测在 Windows 上完成 |
| Python | 3.12 | 已满足 | 用 `uv` 管理虚拟环境 |
| [uv](https://docs.astral.sh/uv/) | ≥ 0.12 | 已满足 | 装依赖（本机在 `Z:\Python\Python_3_12_1\Scripts\uv.exe`） |
| DeepSeek API Key | — | 已满足 | 翻译用；没有也能跑 Fake 引擎验证流程 |
...
把项目从开发机搬到别的机器时，`.env` 不会跟着走（它在 `.gitignore` 里），
所以界面顶上固定有一个「配置」入口。三条设计约定：

1. **密钥明文永不回前端**。后端只回打码串（`sk-0457****ba86`），前端拿不到明文。
2. **留空 = 不修改**。想抹掉已保存的密钥必须点明确的「清除密钥」——否则用户只改模型、
   顺手点了保存，就会把密钥一起清掉。
3. **只提交改动过的字段**，不做整表覆盖；`.env` 里其它内容与注释原样保留。

保存后**立即生效**，不用重启服务：写入文件的同时也更新服务进程的环境变量，
而作业子进程继承该环境并会重新读盘上的 `.env`。

实测（真实浏览器，Playwright）：只改模型 → 密钥字节级未被改动（35 → 35 字符，
SHA256 前后一致）；「当前生效」卡片随即显示新模型，证实无需重启。

> 保存会写到**本项目正在用的那一份 `.env`**，页面上会显示它的绝对路径。
> 装成 wheel 之后（`PROJECT_ROOT` 不再是项目根）会写当前工作目录下的 `.env`。

### 4.2 界面视觉约定

界面的取值集中在 `web/src/styles.css` 的 `:root` 里——颜色、圆角、字号、间距、动效时长、层级
都只有这一个来源，组件里不再出现硬编码的 px。

| 项 | 约定 |
|---|---|
| 圆角 | **刻意分三档**：容器 12px / 控件 8px / 小标签 6px。层级不同就该有不同手感 |
| 正文 vs 界面 | 书的内容（原文、机器译文）用**衬线**（`--font-book`），界面外壳用无衬线。这不是装饰：校对界面里读的是成段的文学作品，衬线更好读，也让"书"和"工具"分得开 |
| 数字 | 统计值、百分比、计数一律 `tabular-nums` 等宽——否则数字刷新时整块会左右抖 |
| 强调色 | 只留给**唯一的主操作**。行内重复动作（校对、下载、保存）一律 ghost 外观 |
| 动效 | 交互元素都有 110–320ms 的过渡；`prefers-reduced-motion: reduce` 下全部关掉 |
...
>
>**开发模式要自己告诉它后端在哪**：`vite.config.ts` 默认把 `/api` 代理到
...
>**实测结论：本地 Qwen3-8B 不适合作主译**。见 [§6.5](#65-引擎对比m3)。
