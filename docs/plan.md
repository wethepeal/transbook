...
**PDF**：✅ **已实现**（`src/transbook/ingest/pdf.py`）——pypdfium2 取文字 + **书签取章节**
+ **列首字下げ切段落**（竖排）/ 首行缩进切段落（横排）。
⚠️ **勘误**：竖排 PDF **不需要列序重排**——pypdfium2 的阅读顺序本身正确
（实测与 EPUB 孪生版逐段比对 200/200 命中）；竖排检测用**字符推进方向**，不要信 PyMuPDF 的"行方向"。
竖排输出的**目标语言是中文，按中文本来的阅读习惯横排**（左→右、上→下），
不保留日文縦書き版式。英文去连字符/连字符合并仍待 M2 补。
...
| **M2** 抽取质量 | **NAV/类名结构识别**（样书 h=0 的坑）、**PDF 输入路径**、**竖排段落还原**、图片、日文分词与 ruby 策略 | 🚧 **进行中**：PDF 输入已完成并验证（段落数 3317 vs EPUB 3388、文本精确命中 92.3%）；剩 PDF 图片提取、英文去连字符、EPUB 前后附页归类 |
...
> **预估 ¥0.5361（flash）/ ¥1.9680（v4-pro）**（空闲时段）。
> 复核命令：`tp translate <workdir> --engine deepseek --dry-run --price-tier idle`（**不花钱**）。
>
> ✅✅ **整本真实测量（2026-09-20，3401 段全量）**：**实际花费 ¥0.9054**
> （输入 228,837 / 输出 169,132 token，114 批，**0 失败**，**16 分钟**，空闲时段定价；
> 高峰约 ¥1.81）。**为 dry-run 预估的 1.69 倍**——差异来自每批系统提示词开销与真实 token 数。
> 你的 ¥20~30 预算 → 实际仅用 **3~4%**。
>
