...
**日期** 2026-09-17　**状态** 🚧 **6/7 完成**（仅 #7「真实 API 调用」待用户配置 `.env`；其余全部实测完毕）
...
**竖排**：本书 **横排**（横排行 155,301 / 竖排行 1,466 ≈ 1%，主要是竖排数字或标题）。
→ ⚠️ **竖排 PDF 的列序重排仍未验证**（这本不是竖排）。需要你另给一本**真正竖排**的 PDF；
或按计划在 M2 用合成样例验证。竖排检测目前依赖 PyMuPDF 的行方向信息（诊断用），
生产路径将改用 pypdfium2 的字符包围盒推断列序（M2 实现）。
...
- ✅ **七项实测完成 6 项**：
  ...
  | 7 | DeepSeek 单章真实成本 | 🚧 **预估已校准 ¥0.54（flash）/¥1.97（v4-pro）**；真实调用待 `.env` |
- ✅ **M1 全链路已提前打通**（抽取 → IR → 预览 → 入库 → 翻译 → **双语/纯中文 EPUB**），
  真实样书端到端跑通（3401 段 / 13 章 / 含 22 图）；47 个测试全绿

**唯一剩余**：`.env` 里填 `DEEPSEEK_API_KEY` → 跑一次真实单章翻译，即可收尾 M0 并进入 M2。
